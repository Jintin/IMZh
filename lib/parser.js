...
function connect(urls, index, callback) {
...
function getWikiTw(text, callback) {
    ...
    connect(["https://zh.wikipedia.org/zh-tw/" + text], 0, function(name, text) {
        if (name) {
            callback(name);
        }
    });
...
function getWikiEn(title, meta, callback) {
    ...
    chrome.storage.local.get(title + ":" + meta, function(result) {
        ...
        connect(list, 0, function(name, text) {
